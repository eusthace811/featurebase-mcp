...
class FeaturebaseAPI {
    ...
    async getPostUpvoters(submissionId, page = 1, limit = 10) {
        return this.request("/posts/upvoters", {
            params: { submissionId, page, limit },
        });
    }
    async addUpvoter(id, email, name) {
        return this.request("/posts/upvoters", {
            method: "POST",
            body: JSON.stringify({ id, email, name }),
        });
    }
    async resolvePostSlug(slug) {
        if (!this.orgUrl) {
    ...
    async getSimilarSubmissions(query, locale = "en") {
        if (!this.orgUrl) {
    ...
    async listChangelogs(params) {
        return this.request("/changelog", { params });
    }
    ...
    async getChangelogSubscribers(params) {
        return this.request("/changelog/subscribers", { params });
    }
    async addChangelogSubscriber(data) {
        return this.request("/changelog/subscribers", {
            method: "POST",
            body: JSON.stringify(data),
        });
    }
    async removeChangelogSubscriber(email) {
        return this.request("/changelog/subscribers", {
            method: "DELETE",
    ...
    setupHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                {
                    name: "get_comments",
                    description: `Get comments for a post or changelog. Available fields: ${commentModel}, replies(${commentModel})`,
                    inputSchema: {
                        type: "object",
                        properties: {
                            submissionId: {
                            ...
                            select: {
                                type: "string",
                                description: 'Fields to return. Examples: "id,content,author(name)" | "content,upvotes,createdAt" | "author(name,email),replies(content)". Leave empty for all fields.',
                            },
                        },
                        required: [],
                    },
                ...
                // Changelog tools
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            select: {
                                type: "string",
                                description: 'Fields to return. Examples: "id,title" | "title,state,categories". Leave empty for all fields.',
                            },
                        },
                        required: [],
                    },
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    // Posts handlers
                    case "list_posts": {
                        const { select, ...apiArgs } = args;
                        const result = await this.api.listPosts(apiArgs);
                        const filtered = select
                            ? {
                                ...result,
                                results: result.results?.map((post) => mask(post, select)) ||
                                    [],
                            }
                            : result;
                        return {
                    ...
                    case "create_post": {
                        const result = await this.api.createPost(args);
                        const filtered = {
                    ...
                    case "update_post": {
                        const result = await this.api.updatePost(args);
                        return {
                            content: [
                                {
                                    type: "text",
                    ...
                    case "get_comments": {
                        ...
                        const filtered = select
                            ? {
                                ...result,
                                results: result.results?.map((comment) => mask(comment, select)) || [],
                            }
                            : result;
                        return {
                            content: [
                    ...
                    case "create_comment": {
                        const result = await this.api.createComment(args);
                        const filtered = {
                            success: result.success,
                            comment: {
                    ...
                    case "update_comment": {
                        const result = await this.api.updateComment(args);
                        return {
                            content: [
                                {
                                    type: "text",
                    ...
                    case "list_changelogs": {
                        ...
                        const filtered = select
                            ? {
                                ...result,
                                results: result.results?.map((changelog) => mask(changelog, select)) ||
                                    [],
                            }
                            : result;
                        return {
                    ...
                    case "create_changelog": {
                        const result = await this.api.createChangelog(args);
                        const filtered = {
                            success: result.success,
                            changelog: {
                    ...
                    case "update_changelog": {
                        const result = await this.api.updateChangelog(args);
                        return {
                            content: [
                                {
                                    type: "text",
                ...
                throw new McpError(ErrorCode.InternalError, `Featurebase API error: ${error.message}`);
            }
        });
    }
    async run() {
